...
export async function generateTomcatMarkdownReport({
  ...
  document.instances.forEach((instance, index) => {
    ...
    const checkRows = [...buildCheckRows(instance), ...hostResourceChecks];
    reports.push({
...
| 巡检项 | 结论 | 采集事实 | 建议 |
| --- | --- | --- | --- |
${rows.filter(({ domain }) => domain !== 'host-resources').map(({ id, conclusion, fact, suggestion }) => `| ${id} | ${conclusion} | ${fact} | ${suggestion} |`).join('\n')}
`;
}
